...
function Home() {
  ...
  useEffect(() =>{    
    axios.get('http://apis-backend.jelastic.saveincloud.net/clientsday')
    .then(res =>{
      setDay(res.data[0].id)
      }).catch(err => console.log(err));

      axios.get('http://apis-backend.jelastic.saveincloud.net/clientsweek')
      .then(res =>{
        setWeek(res.data[0].id)
      }).catch(err =>console.log(err));

      axios.get('http://apis-backend.jelastic.saveincloud.net/clientsmonth')
      .then(res =>{
        setMonth(res.data[0].id)
      }).catch(err =>console.log(err));

      axios.get('http://apis-backend.jelastic.saveincloud.net/clientsdaia')
      .then(res =>{
        setDaia(res.data[0].id)
      }).catch(err =>console.log(err));

      axios.get('http://apis-backend.jelastic.saveincloud.net/clientsdeza')
      .then(res =>{
  ...
  const handleLogout = () => {
    axios.get('http://apis-backend.jelastic.saveincloud.net/logout')
    .then(res => {
